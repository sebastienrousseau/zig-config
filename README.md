...
- [Modern Zig Language Server & Build Standards](#modern-zig-language-server-build-standards) — before and after rule comparison
...
`@sebastienrousseau/zig-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
