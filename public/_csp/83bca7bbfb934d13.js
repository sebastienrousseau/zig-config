...
lp=lm?'/zig-config/'+lm[1]:'/zig-config';
function load(){if(idx)return Promise.resolve();var sp=lm?'/zig-config/'+lm[1]+'/search-index.json':'/zig-config/search-index.json';return fetch(sp).then(function(r){return r.json()}).then(function(d){idx=d.entries||[]}).catch(function(){idx=[]})}
